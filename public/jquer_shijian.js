(function () {
	...
	var endT = '';   //区间结束时间
	// 获取区间时间戳
	function getTimeStemp(startTime, endTime){
		// console.log('当前地图海量点',seaPointList)
		let startTimeStemp = new Date(startTime).getTime()
		let endTimeStemp = new Date(endTime).getTime()
		if(startTimeStemp > endTimeStemp){
			alert('结束时间不得小于开始时间!')
		} else {
			// new DropdownSearch('.custom1', '20191202');  //实例化门店
			let timeStempList = []
			for(let i = 0; i < seaPointList.length; i++){
				let seaCreateTimeStemp = seaPointList[i].createTime ? new Date(seaPointList[i].createTime).getTime() : 0
				if(seaCreateTimeStemp >= startTimeStemp &&  seaCreateTimeStemp <= endTimeStemp) {
					timeStempList.push(seaPointList[i])
				}
			}
			// map.clearOverlays();
			...
			baiduMapController.init(markersStoreList,timeStempList).showStoreMarker()
			baiduMapController.init(markersStoreList,timeStempList).showSeaMarkerPoint()
			console.log(timeStempList)
		}
	};
		        ...
		        sjObj.defaults={
		            ...
		            bindFun:function(){
		                ...
		                sjObj.opt.timeElm.find(".df-ok").on("click",function(){
							...
							if($(sjObj.opt.thisElm).attr('id') === 'inputT'){
								console.log("获取开始确认时间",str)
								startT = str
							}else if($(sjObj.opt.thisElm).attr('id') === 'inputE'){
								console.log("获取结束时间",str)
								endT = str
							}
							console.log(startT , endT)
							if(startT && endT){
								getTimeStemp(startT, endT)
							}
